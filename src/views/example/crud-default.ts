...
import { type BaseRow, type ColumnsItemType, type ColumnsType, CommonProps, type Fields, ReRadioGroup, ReSelect } from '@/components/CRUD'
  ...
  const userId: ColumnsItemType<T, 'userId'> = reactive({
    ...
    componentProps: {
      ...
      onRemoteMethod: (query: string, cancelToken: Ref<CancelTokenSource | undefined>, closeLoading: () => void) => {
        ...
        Http.CrudRequest<{ userId: number, userName: string, realName: string, phoneNumber: string }[]>({
          url: '/user/query',
          method: 'POST',
